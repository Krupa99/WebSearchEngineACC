...
public class SearchWord {
	...
	// Positions and Occurrences of the words
		public static int WordSearch(String data, String word, String fileName) 
		{
			...
			if (Count != 0) 
			{
				System.out.println("Founded in HTML files ==>> " + fileName+" ==>> "+Count+" times"); // Finds from HTML file entered by user
				System.out.println("\n >>>>>>>>>>>>>>>>>>>> ------------------------------- <<<<<<<<<<<<<<<<<<<\n");
		...
		// Merge-sort is used for ranking of the pages
		public static void fileRanking(Hashtable<?, Integer> files, int occur) 
		{
			...
			if (occur != 0) 
			{	
				System.out.println("====>>> Top searched results available <<<====");
				int noOfFetch = 5;
				int j = 0;
				int i=1;
		...
		// Suggests the word as pattern matches
		public static void WordSuggestions(String wordToSearch) 
		{
			String line = " ";
			String regex = "[a-z0-9]+";
			// Create a Pattern object
			Pattern pattern = Pattern.compile(regex);
			// Now will create matcher object.
			...
			for (int i = 0; i < fileArray.length; i++) 
			{
				try 
				{
					Find_Word(fileArray[i], fileNumber, matcher, wordToSearch);
					fileNumber++;
				} 
			...
			Integer allowdistance = 1; // Edit distance allowed
			boolean matchFound = false; // set to true if word found with edit distance equal to allowdistance
			
			int i = 0;
			for (Map.Entry entry : numbers.entrySet()) 
			{
				if (allowdistance == entry.getValue()) 
				{	
					i++;
					if(i==1)
		...
		public static void Find_Word(File sourceFile, int fileNumber, Matcher match, String str)
